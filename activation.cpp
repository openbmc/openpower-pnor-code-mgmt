...
void Activation::createMountFiles()
{
    ...
    if (!fs::exists(mountROFile))
    {
        ...
        mountRO << wantedBy;
        mountRO.close();
        files.push_back(mountROFile.c_str());
    }
    ...
    if (!fs::exists(mountRWFile))
    {
        ...
        mountRW << wantedBy;
        mountRW.close();
        files.push_back(mountRWFile.c_str());
    }
    ...
    if (!fs::exists(mountPRSVFile))
    {
        ...
        mountPRSV << wantedBy;
        mountPRSV.close();
        files.push_back(mountPRSVFile.c_str());
    }
    ...
    method = bus.new_method_call(
            ...
            SYSTEMD_INTERFACE,
            "EnableUnitFiles");
    method.append(files, false, true);
    bus.call_noreply(method);
}
    ...
    if (value == softwareServer::Activation::Activations::Activating)
    {
        ...
        else if (squashfsLoaded == true && rwVolumesCreated == true)
        {
                ...
                (fs::is_directory(PNOR_RO_PREFIX + versionId)))
            {
                ...
                // Create systemd Mount files for RO and RW partitions.
