...
constexpr auto HOST_STATE_OBJ = "xyz.openbmc_project.State.Host";
constexpr auto HOST_STATE_RUNNING =
...
bool ItemUpdater::isVersionActive(std::string versionId)
{
    if (!fs::exists(PNOR_RO_ACTIVE_PATH) || !fs::exists(PNOR_RW_ACTIVE_PATH))
        return false;

    fs::path activeRO = fs::read_symlink(PNOR_RO_ACTIVE_PATH);
    fs::path activeRW = fs::read_symlink(PNOR_RW_ACTIVE_PATH);

    if (!fs::is_directory(activeRO) || !fs::is_directory(activeRW))
        return false;

    if (activeRO.string().find(versionId) == std::string::npos)
        return false;

    if (activeRW.string().find(versionId) == std::string::npos)
        return false;
...
bool ItemUpdater::isHostRunning()
{
    auto mapperCall = bus.new_method_call(
            ...
            MAPPER_INTERFACE,
            "GetSubTree");
    mapperCall.append("/", 0, std::vector<std::string>({HOST_STATE_OBJ}));
    ...
    auto method = bus.new_method_call(HOST_STATE_OBJ,
                                      (mapperResponse.begin()->first).c_str(),
                                      SYSTEMD_PROPERTY_INTERFACE,
                                      "Get");
    method.append(HOST_STATE_OBJ, "CurrentHostState");
    auto response = bus.call(method);
    if (response.is_method_error())
    {
        log<level::ERR>("Error in fetching current Host State");
        elog<InvalidArgument>(xyz::openbmc_project::Common::InvalidArgument::
                              ARGUMENT_NAME("Response"),
                              xyz::openbmc_project::Common::InvalidArgument::
                              ARGUMENT_VALUE(""));
    }
    sdbusplus::message::variant<std::string> currentHostState;
    response.read(currentHostState);
    auto strParam = sdbusplus::message::variant_ns::
                                    get<std::string>(currentHostState);
    return (strParam == HOST_STATE_RUNNING);
}
...
{
    if (isVersionActive(entryId) && isHostRunning()) {
